...
CREATE FUNCTION [dbo].[Fn_GetEntranceFrontOpeningForLD]( @groupId int,@unitId int,@variableId nvarchar(100))
 ...
 select distinct VariableValue   from EntranceConsole eco
   ...
   Left join units ut on ut.SetId=eco.SetId
    where eco.SetId in (Select distinct SetId from units where GroupConfigurationId=@groupId) 
     and el.Front=1 and el.IsDeleted=0   and ecc.VariableType like   '%'+@variableId+'%'  and ut.UnitId=@unitId)a
