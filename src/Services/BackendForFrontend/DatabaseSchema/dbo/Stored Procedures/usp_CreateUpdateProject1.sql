create procedure [dbo].[usp_CreateUpdateProject1]
@ProjectDetailList MiniProjectsDetailsListValuesData1 READONLY
...
begin try
		...
		begin
			...
			select OpportunityId,0 as VersionId,'' as QuoteId , @IsNewQuote as IsNewQuote from Projects where OpportunityId  = (select distinct(OpportunityId) from @ProjectDetailList)
		...
		begin
			...
			BEGIN
				...
				where QuoteValues.OpportunityId  = (select distinct(OpportunityId) from @ProjectDetailList)
				and QuoteValues.VersionId=((select distinct(VersionId) from @ProjectDetailList))
			select OpportunityId,VersionId,QuoteId , @IsNewQuote as IsNewQuote from Quotes where OpportunityId  = (select distinct(OpportunityId) from @ProjectDetailList)
				and VersionId=((select distinct(VersionId) from @ProjectDetailList))
			--set @Result=(select QuoteId from Quotes where OpportunityId  = (select distinct(OpportunityId) from @ProjectDetailList)
			--	and VersionId=((select distinct(VersionId) from @ProjectDetailList)))
			return 0
			...
			BEGIN
				...
				set @IsNewQuote =1
				select OpportunityId,VersionId,QuoteId, @IsNewQuote as IsNewQuote from Quotes where id=SCOPE_IDENTITY()
		...
		begin
			...
			set @IsNewQuote = 1
			select OpportunityId,VersionId,QuoteId ,@IsNewQuote as IsNewQuote 
			from Quotes --where OpportunityId = 'SC-70'
