...
CREATE PROCEDURE [dbo].[usp_SaveUpdateGroupHallFixtures1]
	@GroupId int,
	@ConsoleNumber int,
...
Begin
	BEGIN TRY
        ...
        if( @FixtureType in('Fire_Service','Emergency_Power','Appendix_H/O_Signage','Code_Blue_Key_Switch','VIP_Service_Switch','Medical_Emergency_Key_Switch',
		...
		begin
			if( @FixtureType in('Hall_Call_Lockout','Hall_Call_Registration'))
			begin
					--deselect the front openings which are overwritten
			update GroupHallFixtureLocations 
			set Front = 0
			...
			on gl.FloorDesignation=gv.FloorDesignation and gl.HallStationName=gv.HallStationName
			where gc.GroupHallFixtureConsoleId <> @groupHallFixtureConsoleId and gv.Front=1 and gl.Front=1
			and gc.GroupId=@GroupId and gc.FixtureType in ('Hall_Call_Lockout','Hall_Call_Registration')
			--deselect the Rear openings which are overwritten
			update GroupHallFixtureLocations 
			set Rear = 0
			from 
			...
			where GroupHallFixtureConsoleId not in(
										select distinct el.GroupHallFixtureConsoleId from GroupHallFixtureLocations el 
										join GroupHallFixtureConsole ec on ec.GroupHallFixtureConsoleId=el.GroupHallFixtureConsoleId
										where (Front=1 or Rear=1) and GroupId=@GroupId and FixtureType in('Hall_Call_Registration','Hall_Call_Lockout')
										group By el.GroupHallFixtureConsoleId
									   )and  GroupId=@GroupId and FixtureType in('Hall_Call_Lockout','Hall_Call_Registration')
			order by GroupHallFixtureConsoleId desc
		open db_cursor  
		fetch next from db_cursor into @grouphallfixtureconsoleNumber
		while @@FETCH_STATUS = 0  
			begin  
				  ...
				  declare @sp_consoleNumber1 int
				  declare @sp_fixtureType1 nvarchar(50)
				  select @sp_consoleNumber1=ConsoleNumber,@sp_fixtureType1=FixtureType
				  from GroupHallFixtureConsole where GroupHallFixtureConsoleId=@grouphallfixtureconsoleNumber
				  exec [dbo].[usp_DeleteGroupHallFixtureConsole]@GroupId,@sp_consoleNumber1,@sp_fixtureType1,@historyTable,@createdBy,@sp_result1
				  fetch next from db_cursor into @grouphallfixtureconsoleNumber
			end
			end
			else
		...
		while @@FETCH_STATUS = 0  
			begin  
				  ...
				  exec [dbo].[usp_DeleteGroupHallFixtureConsole]@GroupId,@grouphallfixtureconsoleNumber,@FixtureType,@historyTable,@createdBy,@sp_result2
				  fetch next from db_cursor into @grouphallfixtureconsoleNumber
			end 
