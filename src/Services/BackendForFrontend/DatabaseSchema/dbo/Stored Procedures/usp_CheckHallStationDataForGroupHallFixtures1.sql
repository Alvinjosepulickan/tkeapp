CREATE Procedure [dbo].[usp_CheckHallStationDataForGroupHallFixtures1] --1133,'c2dUser','ETA/ETD' declare @hallStationVariables insert @hallStationVariables values('Parameters_SP.HS2R_SP','','')
...
BEGIN
		...
		BEGIN
			...
			select distinct hallstation.consoleId,
							...
							case when @fixtureStrategy='ETA/ETD' then 
								...
								case when @minconsoleId=@consoleId 
									then
									case when ghfc.FixtureType = 'Fire_Service' or ghfc.FixtureType = 'Emergency_Power'
										then case when ghl.FloorDesignation = @mainEgress
												then ol.Front
												else 0
												end
   ...
   declare db_cursor cursor for
			...
			where GroupHallFixtureConsoleId not in(
										...
										where (Front=1 or Rear=1) and GroupId=@groupConfigurationId
										group By el.GroupHallFixtureConsoleId 
									   )and  GroupId=@groupConfigurationId 
