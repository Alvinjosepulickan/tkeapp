CREATE procedure [dbo].[usp_GetVariableByQuoteId] --'US-2021-100008'
(
 @quoteId nvarchar(50)
)
as
begin
	...
	begin try
		 select distinct BuildingId,BuindingType,BuindingValue from BuildingConfiguration
		  where  BuildingId in (select Id from Building where QuoteId=@quoteId and IsDeleted=0) and IsDeleted=0

		 select distinct  BuildingId,Variabletype,[Value] from BldgEquipmentConfiguration
		  where BuildingId in (select Id from Building where QuoteId=@quoteId and IsDeleted=0)
   
   
		   select distinct  BuildingId,bcc.ConsoleId,Variabletype,[Value] from BldgEquipmentConsoleCnfgn bcc Left Join BldgEquipmentConsole bc on bcc.ConsoleId= bc.ConsoleId
		  where  bcc.ConsoleId in (select ConsoleId from BldgEquipmentConsole where BuildingId in (select Id from Building
		 ...
		 select distinct  BuildingId,GroupId,GroupName from GroupConfiguration where IsDeleted=0 and BuildingId in (select Id from Building where QuoteId=@quoteId and IsDeleted=0)
		 ...
		 select GroupConfigurationId,GroupName,BuildingId,ControlLocationType,ControlLocationValue from ControlLocation cl Left Join GroupConfiguration gc
		  on cl.GroupConfigurationId= gc.GroupId where GroupConfigurationId in (select GroupId from GroupConfiguration where BuildingId in (select Id from Building where QuoteId=@quoteId and IsDeleted=0)and IsDeleted=0)
		 ...
		 Left Join GroupConfiguration gc on us.GroupConfigurationId=gc.GroupId
		 LEFT JOIN openinglocation ol ON ol.unitid = us.unitid AND ol.isdeleted = 0
		 where  uc.SetId in (select SetId from Units where GroupConfigurationId in (select GroupId from GroupConfiguration
		 where BuildingId in (select Id from Building where QuoteId=@quoteId and IsDeleted=0))and IsDeleted=0)
		...
		select GroupId,GroupConfigurationType,GroupConfigurationValue from GroupConfigurationDetails where GroupId in (select distinct  GroupId from GroupConfiguration where IsDeleted=0 and BuildingId in (select Id from Building where QuoteId=@quoteId and IsDeleted=0))
	end try
	begin catch
		RAISERROR('something went wrong',1,1)
		EXEC usp_Log_ProcedureCall @ObjectID = @@PROCID,
