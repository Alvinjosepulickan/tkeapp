...
CREATE PROCEDURE [dbo].[usp_GetDrawingsByGroupIdForRefresh]--434,'Te-2021-000001' --196,'US-2021-000008' --'ADIA-1N16VG8'
 @groupId int
...
Begin
  ...
  declare @quoteStatus nvarchar(200)
	...
	SET @quoteStatus = (select QuoteStatusId from Quotes where QuoteId = @quoteId)
     ...
     DECLARE db_cursor1 CURSOR FOR Select 
              ...
              Left Join GroupConfiguration gc on bg.Id=gc.BuildingId
               Where QuoteId= @quoteId and gc.IsDeleted=0 and bg.IsDeleted=0 and id <> ''
	 ...
	 DECLARE db_cursor CURSOR FOR select WorkFlowStatus from groupconfiguration where buildingId in (select id from building where quoteId =@quoteId and IsDeleted = 0) and IsDeleted=0
	...
	,
	  case when (Select top 1 s.StatusId  from FieldDrawingMaster fdm
	    Left Join Status s on fdm.StatusKey=s.StatusKey
		  where fdm.IsDeleted=0 and s.IsDeleted=0 and fdm.QuoteId=@quoteId and fdm.GroupId=gc.GroupId
   ...
   Left Join Status s on s.StatusKey=gc.WorkflowStatus
      Where QuoteId=@quoteId and groupId= @groupId and gc.IsDeleted=0 and us.IsDeleted=0 and bg.IsDeleted=0
